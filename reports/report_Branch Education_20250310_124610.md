...
## 处理统计
- 处理时间：2025-03-10 12:46:10 至 2025-03-10 12:46:10
- 音频提取耗时：0.00秒
- 转录耗时：0.00秒
- 翻译耗时：0.00秒
- 内容章节数：1
...
你认为你的显卡在运行具有极其逼真图形的视频游戏时，每秒会进行多少次计算？ 也许是一亿。 好吧，每秒一亿次计算是运行1996年的《马里奥64》所需的速度。 我们需要更多电力。 也许每秒一百亿次计算。 好吧，那么你将拥有一台可以在2011年运行Minecraft的电脑，而要运行像《赛博朋克2077》这样最逼真的视频游戏，你需要一块每秒能够执行约36万亿次计算的显卡。 这是一个难以想象的大数字。 所以让...
</div>


## 关键观点

1. 你认为你的显卡在运行具有极其逼真图形的视频游戏时，每秒会进行多少次计算？
2. 但是，当我们查看安装在主板上的cpu或中央处理单元时，我们发现一个只有24个核心的集成电路或芯片。
3. 另一个关键区别是，飞机要灵活得多，因为它们可以运载乘客、包裹或集装箱，并且可以在成千上万个机场中的任何一个起飞和降落。
4. 观看另一个由克利奥妮制作的。
...
译文：你认为你的显卡在运行具有极其逼真图形的视频游戏时，每秒会进行多少次计算？  
...
译文：在我们深入探讨GPU的所有组成部分之前，首先了解一下这张显卡内部的GPU和CPU之间的区别。  
...
（注意：为了保持语句通顺，翻译时略微调整了语序和表达方式，但保留了原始格式。）  
...
译文：另一个关键区别是，飞机要灵活得多，因为它们可以运载乘客、包裹或集装箱，并且可以在成千上万个机场中的任何一个起飞和降落。  
...
译文：具体来说，该芯片被划分为七个图形处理簇（GPCs），在每个处理簇内包含十二个流式多处理器（SMs）。  
...
注意：为了保持语句的流畅性和准确性，我对句子结构做了一些调整，但保留了原文的意思和格式。  
...
译文：既然我们了解了这个芯片内的计算资源，一个相当有趣的事实是，三张80显卡、三张90显卡、三张80 Ti显卡和三张90 Ti显卡都使用相同的GA102芯片设计作为它们的GPU。  
...
译文：这可能有悖常理，因为它们价格不同，发布年份也不同，但事实就是这样，为什么会这样呢？  
...
译文：好吧，在制造过程中，有时会出现图案错误、灰尘颗粒或其他制造问题，导致电路损坏并产生缺陷区域。  
...
译文：通过拥有一个设计高度重复的GPU，一个内核中的小缺陷只会损坏那个特定的流式多处理器电路，而不会影响芯片的其他区域。  
...
译文：这三块90 Ti显卡拥有完美的GA102芯片，所有10752个CUDA核心均能正常工作。  
...
译文：此外，不同的显卡在其最大时钟频率以及支持GPU的显存的数量和代际上有所不同，我们将在稍后讨论这一点，因为我们一直专注于这种GA102 GPU芯片的物理架构。  
...
译文：这一段包含50,000个晶体管的区域执行的操作是a乘以b再加上c，这被称为融合乘加操作或FMA，是图形卡最常执行的操作。  
...
译文：这个核心的其他部分负责处理负数并执行其他简单功能，如位移和位掩码，以及收集和排队传入的指令和操作数，然后累积和输出结果。  
...
译文：但是，如果您想知道GPU是如何处理更复杂的操作，比如除法、平方根和三角函数的，那么这些计算器操作是由特殊功能单元执行的，而在每个流式多处理器中仅能找到四个这样的单元，因此它们的数量远远少于其他单元。  
...
译文：既然我们已经了解了单个核心的内部结构，让我们放大一点，看看GA102芯片的其他部分。  
...
注：这里的“envy link controllers”直译为“羡慕链接控制器”，这可能是一个特定技术名词或拼写错误，在常规语境中并不常见，因此保留了其英文形式。如果这是一个特定的技术术语，请根据具体上下文进行适当调整。  
...
译文：现在我们已经了解了这个GA 102 GPU的物理架构，让我们放大视野，看看显卡内部的其他部分。  
...
译文：也许除了GPU之外，一些最重要的组件是24GB的图形内存芯片，从技术上讲，这些芯片被称为GDDR6 XS RN，由美光制造，而美光也是本视频的赞助商。  
...
注：原句似乎有些语法和表达上的问题，因此翻译时做了一定的语义调整以使其更通顺。如果需要严格按照原文翻译，请告知。  
...
译文：因此，为了渲染视频游戏，不同的场景块会不断在图形内存和GPU之间传输，因为核心不断地进行数万亿次的计算。  

**00:13:01 - 00:13:10**  
原文：A second gpuare data hungry machines and need to be continuously fed terabytes upon terabytes of data.  
译文：第二个gpu是数据 hungry 的机器，需要持续喂给它TB又TB的数据。 

注：原文中“gpuare”似乎是拼写错误，应为“GPUs are”。此外，“data hungry”直译为“数据饥饿的”，在中文中通常表达为“数据密集型”或“需要大量数据”。为了更符合中文表达习惯，可以调整为：

第二个GPU是数据密集型的机器，需要持续不断地输入大量数据，一次又一次地以TB为单位。  
...
注：此处“bus swift”直译为“总线 swift”，但根据上下文和专业术语，“swift”可能是指“快速传输”或特定技术名称，如果这是专有名词或特定技术术语，建议确认其准确含义。  

**00:13:27 - 00:13:34**  
原文：And the total data that can be transferred or the bandwidth is about 1.15 tb a second.  
译文：而且每秒可以传输的总数据量或带宽约为1.15 TB。  
...
译文：然而，为了提高数据传输速率，GDDR6 和最新的图形内存 GDDR7 在总线线路上使用多个电压电平（而不仅仅是0和1）来发送和接收数据。  
...
译文：例如，gdd R seven 使用三种不同的编码方案，将二进制位组合成三元数字，或者用零、一和负一电压的 pam 三个符号。  
...
译文：以下是三元编码方案，说明了如何将三个二进制位编码为两个三元位，该方案与将十一位编码为七个三元位的方案结合，最终实现使用176个三元位发送276个二进制位。  
...
译文：下次您购买AI加速系统时，确保它使用的是微米级、HBM3E，其功耗比竞争产品低30%。  
...
译文：但是，除非您正在建设人工智能数据中心，否则您可能不会考虑购买这些系统之一，它们的价格在25,000到40,000美元之间，并且需要等待几年才能交付。  
...
译文：现在我们已经探索了这个显卡和GPU内部的许多物理组件，接下来让我们探讨计算架构，并看看像视频游戏图形和比特币挖掘这样的应用是如何运行所谓的令人尴尬的并行操作的。 

请注意，"embarrassingly parallel"在计算机科学中是一个专业术语，通常直译为“令人尴尬的并行”，但在中文语境下有时会根据具体场景调整表述，以确保理解准确。这里我保留了直译方式。  
...
译文：虽然听起来可能是个滑稽的名字，但令人尴尬的并行（embarrassingly parallel）实际上是计算机问题的一种技术分类，在这种分类中，将问题划分为并行任务所需的努力很少或几乎不需要，而视频游戏渲染和比特币挖矿很容易就属于这一类别。  
...
译文：本质上，GPU 使用一种称为 SIMD（单指令多数据）的原则来解决令人尴尬的并行问题，其中相同的指令或步骤会在成千上万到数百万个不同的数据上重复。  
...
译文：如你所知，这张桌子上的牛仔帽是由大约28,000个三角形组成的，这些三角形是由大约14,000个顶点连接而成的，每个顶点都有x、y和z坐标。  
...
译文：sembly 和令人尴尬的并行程序的关键在于，这数以百万计的计算中的每一个都与其他计算没有任何依赖关系，因此所有这些计算都可以分配给 GPU 的数千个内核，并可以并行完成。请注意，这里可能有表述不准确的地方，“sembly”一词似乎有误，您可能指的是“assembly”（装配、组装），而“embarrassingly parallel programs”通常翻译为“令人尴尬的并行程序”，这是指那些非常容易实现并行化的程序。如果您有具体的上下文或需要进一步澄清，请告知。  
...
译文：线程被捆绑成每32个一组，称为 warp（线程束），并且相同的指令序列会被发送给 warp 中的所有线程。  
...
译文：所有这些计算都由 gigatthread 引擎进行管理或调度，该引擎能够高效地将线程块映射到可用的流式多处理器上。  
...
译文：一个重要的区别是，在 sidy 架构中，所有 32 个线程在战争中都遵循相同的指令，并且彼此同步，就像一队士兵一起行动一样。 

注：原文中的 "sidy" 和 "war" 可能是特定术语或拼写错误，根据上下文我将其理解为一种架构和线程组的形式。如果你有更具体的背景信息，请告知以便更准确地翻译。  
...
译文：sid 和 sit 之间的区别在于，尽管两者都向每个线程发送相同的指令集。  
...
译文：另外，借助 sympty，流式多处理器中的所有线程使用共享的 128 千字节一级缓存，因此一个线程输出的数据可以随后被另一个线程使用。  
...
译文：从 sim d 改进到 sim t，在遇到依赖于数据的条件分支所产生的扭曲发散时，提供了更大的灵活性，并且使线程更容易重新聚合以达到屏障同步。  
...
（注：为了保持原始格式，此处的“newer architectures of gpus”未翻译成中文，如果需要完全翻译可以改为“GPU的新架构”。）  
...
译文：另有一个注解是，虽然你可能认为术语“warp”来源于曲速引擎，但实际上它来自于织造，具体来说是jekar织机。  
...
译文：分支教育的梦想是制作免费且易于访问的、视觉上引人入胜的教育视频，深入探讨科学、工程和技术创新等多个主题，然后将多个视频组合成完全免费的工程课程，供高中生和大学生使用。  
...
译文：所以我们已经探讨了单指令多线程在视频游戏中的应用，现在让我们简要讨论一下为什么最初使用GPU进行比特币挖矿。  
...
译文：你可以这样理解这个算法，它就像一个彩票号码生成器，你不能选择彩票的号码，但是根据输入的数据，SHA256算法会生成一个随机的彩票号码。  
...
译文：吐币 mining 彩票的赢家是第一个随机生成的彩票号码，其前80位全部为零，而其余的176位值则不重要。 

注意：原文中的 "spit coin" 可能是指某种虚拟货币或特定情境下的术语，这里翻译为“吐币”以保持与原文一致，但请根据实际情况确认是否需要调整术语的翻译。  
...
译文：然而，如今，装有六个或更多应用特定集成电路的计算机每秒可以执行250万亿次哈希运算，这相当于两块600系列显卡的性能，从而使显卡在挖掘比特币时看起来就像在ASIC矿机（即应用特定集成电路矿机）旁边的一把勺子。  
...
译文：此值等于第一个矩阵的第一行的值与第二个矩阵的第一列的值相乘后再相加，然后加上第三个矩阵的相应值。因为三个输入矩阵的所有值同时准备好，张量核心会并发完成所有的矩阵乘法和加法计算。  
...
译文：最后，还有光线追踪核心，关于这部分内容我们在另一个已经发布的视频中进行了探讨。  
...
译文：我们感谢所有 Patreon 和Tube会员赞助商对我们的视频提供支持。  
...
译文：这是分支教育，我们创建3D动画，深入探讨推动现代世界发展的技术。  
